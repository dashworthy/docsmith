...
Lives in [`skills/docsmith/`](../../../skills/docsmith) (the `docsmith:docsmith` skill owns the package). End-user usage (authoring a doc, running the CLI) is in [`skills/docsmith/README.md`](../../../skills/docsmith/README.md); this doc is the architecture and the invariants a future change must respect.
...
| Components | `src/pdf/components/*.tsx` | The presentational library (see `skills/docsmith/README.md` for the full list). Color only through `useTw()`. |
...
| Run wrapper | `src/pdf/render.ts` | `renderRun({runDir, themes?})` — renders a `.docsmith/<run>/pdf.tsx` to `<run>/pdf-light.pdf` + `<run>/pdf-dark.pdf` (both themes by default), reusing `generatePdf`; the `render <run-dir>` CLI. The `docsmith:docsmith` skill's authoring entry point. |
...
- **A whole run — `render.ts`.** `node --import tsx src/pdf/render.ts <run-dir> [--theme light|dark]` is the `docsmith:docsmith` skill's authoring entry point. A document is authored **outside** the package, in the invoking project at `.docsmith/<run>/pdf.tsx` (importing the bare `@docsmith/docsmith`), and the wrapper renders **both** themes into that run dir as `pdf-light.pdf` / `pdf-dark.pdf`.

Because `node --import tsx` only applies the package's JSX runtime and module resolution to files under the tsconfig `include` (`src`/`test`), the wrapper **stages a temp copy of the authored doc under `src/docs/`**, renders it there — where its bare `@docsmith/docsmith` import resolves by **package self-reference** (the `exports` field) — and removes the copy in a `finally`. The authored file never learns where the package lives; it names only the stable specifier.
...
- **The authored doc names only `@docsmith/docsmith`, never a package path.** The `exports` field (`"." → ./src/pdf/index.ts`) is what makes that bare import legitimate by self-reference, and the `render` wrapper stages the doc under `src/docs/` to render it there — do not remove `exports`, and keep the staging inside the tsconfig `include`. An authored `pdf.tsx` must be self-contained: only `@docsmith/docsmith`, no relative imports or local asset files (assets are inlined via `highlightCode`/`rasterizeMermaid`), because staging moves the file away from its own directory.
