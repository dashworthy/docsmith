...
| `Footer` | `{ lines: string[] }` | A muted mono footer (provenance, source, generated-by). Last, once. |
...
| `Flow` | `{ lanes: Lane[] }` where `Lane = { label?: string; tone: 'bad'\|'good'; steps: { label: string; tone?: 'bad'\|'good' }[] }` | Horizontal step lanes with arrows — a pipeline read left-to-right, toned good/bad. Use for a short linear flow where a full mermaid diagram is overkill. |
