...
| `Footer` | `{ lines: string[] }` | A muted mono footer for the document's *own* provenance — author, date, confidentiality. Optional: omit unless the source or user calls for one, and never use it for tool/skill attribution (no "Generated by …"). Last, once. |
